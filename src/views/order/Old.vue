...
// 已完成订单列表
let finishedList = ref()
onMounted(async () => {
  const res = await axios.get('http://localhost:12580/order')
  finishedList.value = res.data.filter((item: any) => {
    return item.isFinished === true
  })
  console.log('订单列表',finishedList.value)
  for (let i = 0; i < finishedList.value.length; i++) {
    let api = finishedList.value[i].isProduction === true ? 'production/' : 'groceryHot/'
    axios.get('http://localhost:12580/' + api + finishedList.value[i].goodId).then((result) => {
      finishedList.value[i].picture = (finishedList.value[i].isProduction === true) ? result.data.goodsPicSwiper[0] : result.data.hotPic[0]
    })
  }
})
</script>
